...
# Data to insert
daily_game_data = [
    ("2025-06-03", "South Korea", 1),
    ("2025-06-04", "China", 1),
    ("2025-06-05", "Suriname", 1),
    ("2025-06-06", "Somalia", 1),
    ("2025-06-07", "Cameroon", 1),
    ("2025-06-08", "Ethiopia", 1),
    ("2025-06-09", "United States of America", 1),
...
def update_database():
    ...
    for game_date, country in daily_game_data:
        cursor.execute(
            """
            INSERT OR REPLACE INTO daily_game (game_date, country, rotation)
            VALUES (?, ?, ?)
        """,
            (game_date, country),
        )
    ...
    for game_date, successes, failures in game_stats_data:
        ...
        print(
            f"Inserted/Updated: {game_date} -> successes: {successes}, failures: {failures}"
        )
